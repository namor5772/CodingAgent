...
Minimal desktop demo: a window titled **Hello World** with a two-tab layout. Tab **one** plays a short looping animation of a stick figure with a top hat and a simple profile face (eye, nose, mouth) walking naturally across a dark canvas, followed by a stick-figure dog trotting behind him; tab **two** plays a looping chase scene — a stick-figure dog runs in from the left after a bird standing on the ground, the bird takes off just before the dog reaches it and flies on just ahead of the dog, all left to right, restarting once both leave the right edge (all three apps). Plus Desktop shortcuts with a custom icon.
...
| `hello_world_macos.mm` | macOS Cocoa/AppKit C++ clone of the same two animations. |
...
- UI baseline is the **two-tab layout** in all three apps: tab **one** hosts the walker animation, tab **two** hosts the dog-chases-bird animation (Tk `ttk.Notebook`, Win32 `WC_TABCONTROL` with child windows toggled on `TCN_SELCHANGE`, macOS `NSTabView`). Keep tab structure and labels in parity across implementations.
...
- Chase-scene invariants (tab two; all three apps): the dog is the same figure as tab one (`dog_pose`/`DogTrotPose` shared via `_draw_dog`/`DrawDog`) driven by a chase phase (+0.065/frame) at `CHASE_SPEED` 3.2 px/frame (400px baseline, same width scaling), with a two-stage speed-up: x1.6 once the dog passes halfway from its `-60` spawn to the perched bird (`chase_dog_x >= (-60 + w*0.62)/2`), plus a further x1.3 (2.08x total) once the nose is `60*scale` from the bird (`chase_dog_x >= w*0.62 - 91*scale`, just before the `30*scale` takeoff trigger); both boosts latch by position, persist to lap end, clear on respawn, and also raise the fleeing bird's speed since it derives from the same `speed`; ground line at the same height as tab one (`h*0.55 + 76*scale`); bird perched at `w*0.62` on the ground until the dog nose (`dog_cx + 31*scale`) closes within `30*scale`, then climbs away at ~45 deg (rise per frame equals its horizontal `speed + 0.2*scale`; no cruise cap) so it stays just ahead of the dog and exits the top of the scene; wings flap as two `limb_end` strokes at `2.36 - 1.1*sin(flap)` (far wing +0.9 rad phase, shorter), flap +0.35/frame; cycle resets when the bird passes `w + 30` and the dog's tail clears the right edge — dog respawns at `-60`, bird perched again.
